...
from tkinter import ttk  # импортирую ttk (themed tkinter) - это современные виджеты с улучшенным внешним видом
import sqlite3  # импортирую модуль sqlite3 для работы с базой данных SQLite
import time  # импортирую модуль time для работы со временем (задержки, измерение времени)
...
class Module2(tk.Frame):  # создаю класс Module2, который наследуется от tk.Frame (это базовый контейнер для виджетов)
    def __init__(self, parent, app=None):  # конструктор класса, принимает parent (родительский виджет) и app (ссылка на приложение)
        ...
        self.app = app  # сохраняю ссылку на основное приложение в атрибуте self.app
        
        # Размер модуля будет адаптироваться под размер окна
        self.pack(fill="both", expand=True)  # размещаю модуль, растягивая на всю доступную область и позволяя расширяться
        
        # Находим ссылку на основное приложение, если не передана
        if self.app is None:  # если ссылка на приложение не была передана при создании объекта
            self.find_app_reference()  # вызываю метод для поиска ссылки на основное приложение
        
        # Переменные для работы с БД
        self.update_interval = 200  # 5 раз в секунду  # устанавливаю интервал обновления в миллисекундах (200 мс = 0.2 сек)
        self.update_task = None  # создаю переменную для хранения задачи обновления (пока не назначена)
        self.data_loaded = False  # флаг, показывающий загружены ли данные (изначально False - не загружены)
        self.last_values = {}  # Словарь для хранения последних значений  # создаю пустой словарь для сравнения изменений
        self.current_values = {}  # Словарь для хранения текущих значений  # создаю пустой словарь для текущих данных
        self.auto_update = True  # флаг автообновления (изначально включено)
        self.active_table = "raw_data"  # По умолчанию используем таблицу raw_data  # устанавливаю имя активной таблицы БД
        self.conn = None  # Соединение с БД  # переменная для хранения соединения с базой данных (пока не установлено)
        self.highlighted_indicators = set()  # Множество для отслеживания выделенных индикаторов  # создаю пустое множество для отслеживания какие индикаторы подсвечены
    ...
    def setup_ui(self):
        ...
        self.auto_update_button = tk.Button(self.buttons_frame, text="Авто", 
                                          ...
                                          bg="lightgreen" if self.auto_update else "lightgray",
