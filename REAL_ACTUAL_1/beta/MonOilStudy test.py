import tkinter as tk  # импорт tkinter для работы с графическим интерфейсом  # импортирую модуль tkinter и сокращаю его название до tk для удобства
import os  # импортирую модуль os для работы с операционной системой (файлы, папки, пути)
import sqlite3  # импортирую модуль sqlite3 для работы с базой данных SQLite
from module1 import Module1  # Импортируем модуль 1  # импортирую класс Module1 из файла module1.py
from tkinter import messagebox  # импортирую messagebox для показа всплывающих сообщений пользователю
from module3 import Module3  # импортирую класс Module3 из файла module3.py

class ModuleTile(tk.Frame):  # класс moduletile, наследуется от tk.frame  # создаю класс ModuleTile, который наследует свойства от tk.Frame
    def __init__(self, parent, module_id, *args, **kwargs):  # инициализатор, принимает родителя, номер модуля и доп. параметры  # конструктор класса, получает родительский виджет, номер модуля и дополнительные параметры
        super().__init__(parent, *args, **kwargs)  # вызываем конструктор родительского класса  # вызываю конструктор родительского класса tk.Frame
        self.module_id = module_id  # сохраняем номер модуля в атрибуте  # сохраняю номер модуля в переменной объекта
        self.configure(borderwidth=2, relief="solid", bg="white")  # задаем рамку 2px, сплошной стиль и белый фон  # настраиваю внешний вид: рамка 2 пикселя, сплошная линия, белый фон
        
        # Минимальный размер для лучшей видимости контента
        self.config(width=350, height=220)  # устанавливаю минимальный размер плитки 350x220 пикселей
        # Позволяем модулю расширяться при увеличении окна
        self.pack_propagate(True)  # разрешаю плитке изменять размер в зависимости от содержимого
        
        # Устанавливаем минимальные размеры, но позволяем расширение
        self.grid_propagate(True)  # разрешаю сетке внутри плитки изменять размеры
        
        # Получаем ссылку на основное приложение
        app = self.master.master  # получаю ссылку на главное окно приложения через двойной master (родитель родителя)
        ...
        if hasattr(app, 'db_connected') and app.db_connected:  # проверяю есть ли у приложения атрибут db_connected и подключена ли БД
            self.locked = False  # если БД подключена, модуль не заблокирован
        else:  # иначе
            self.locked = module_id not in [4, 7, 8, 9]  # блокирую все модули кроме 4, 7, 8, 9
...
class Application(tk.Tk):  # класс приложения, наследуется от tk.tk
    def __init__(self):  # инициализатор приложения
        ...
        # Проверяем и создаем папку для отчетов при запуске приложения
        self.check_and_create_reports_folder()
    ...
    def check_and_create_reports_folder(self):
        """
        Проверяет наличие папки 'reports' в директории программы.
        Если папки нет - создает её и сообщает пользователю.
        """
        try:
            # Получаем путь к директории программы
            program_dir = os.path.dirname(os.path.abspath(__file__))
            reports_folder_path = os.path.join(program_dir, "reports")
            ...
            if not os.path.exists(reports_folder_path):
                ...
                print(f"📁 Создана папка для отчетов: {reports_folder_path}")
                
                # Показываем уведомление пользователю
                messagebox.showinfo(
                    "Папка отчетов создана", 
                    f"Автоматически создана папка для сохранения отчетов:\n{reports_folder_path}\n\n"
                    "Все отчеты системы будут сохраняться в этой папке."
                )
            else:
                print(f"📁 Папка отчетов уже существует: {reports_folder_path}")
                
        except Exception as e:
            # В случае ошибки выводим предупреждение, но не прерываем запуск программы
            print(f"❌ Ошибка при проверке/создании папки отчетов: {e}")
            messagebox.showwarning(
                "Предупреждение",
                f"Не удалось создать папку для отчетов:\n{e}\n\n"
                "Отчеты будут сохраняться в основной папке программы."
            )
