import tkinter as tk  # импортирую модуль tkinter для создания графического интерфейса, сокращаю его название до tk
import sqlite3  # импортирую модуль sqlite3 для работы с базой данных SQLite
import os  # импортирую модуль os для работы с операционной системой (файлы, папки)
from tkinter import messagebox, Toplevel, simpledialog, StringVar, filedialog  # импортирую конкретные части из tkinter:
# messagebox - для показа всплывающих сообщений
# Toplevel - для создания новых окон
# simpledialog - для простых диалоговых окон
# StringVar - для переменных строкового типа, связанных с интерфейсом
# filedialog - для диалогов выбора файлов

class Module1(tk.Frame):  # создаю класс Module1, который наследуется от tk.Frame (это базовый контейнер для виджетов)
    def __init__(self, parent):  # конструктор класса, принимает parent (родительский виджет)
        super().__init__(parent, bg="white")  # вызываю конструктор родительского класса и устанавливаю белый фон
        self.parent = parent  # сохраняю ссылку на родительский виджет в атрибуте self.parent
        
        # Store main application reference
        self.app = None  # создаю атрибут для хранения ссылки на главное приложение
        self.find_app_reference()  # вызываю метод для поиска ссылки на главное приложение
        
        # Local variables
        self.local_conn = None  # создаю атрибут для хранения локального соединения с базой данных
        self.selected_table = None  # создаю атрибут для хранения выбранной таблицы
        ...
        self.main_layout = tk.Frame(self, bg="white")  # создаю основной фрейм для размещения элементов с белым фоном
        self.main_layout.pack(fill="both", expand=True)  # размещаю фрейм, растягивая его на всю область и позволяя расширяться
        
        # Left side for cells
        self.left_frame = tk.Frame(self.main_layout, bg="white")  # создаю левый фрейм для ячеек с белым фоном
        self.left_frame.pack(side="left", fill="both", expand=True)  # размещаю левый фрейм слева, растягивая и позволяя расширяться
        
        # Right side for controls
        self.right_frame = tk.Frame(self.main_layout, bg="white")  # создаю правый фрейм для элементов управления с белым фоном
        self.right_frame.pack(side="right", fill="y", padx=5)  # размещаю правый фрейм справа, растягивая по вертикали, с отступом 5 пикселей
        
        # Title at the top
        self.title = tk.Label(self.left_frame, text="Статус ячеек нефтепровода",  # создаю заголовок с текстом
                            font=("Arial", 12, "bold"), bg="white")  # устанавливаю шрифт Arial, размер 12, жирный, белый фон
        self.title.pack(pady=2)  # размещаю заголовок с вертикальным отступом 2 пикселя
        
        # Create a frame for cells grid
        self.cells_frame = tk.Frame(self.left_frame, bg="white")  # создаю фрейм для сетки ячеек с белым фоном
        self.cells_frame.pack(fill="both", expand=True, padx=5, pady=2)  # размещаю фрейм, растягивая на всю область с отступами
        
        # Create cells grid (8x5 = 40 cells)
        self.cell_indicators = []  # создаю пустой список для хранения индикаторов ячеек
        for row in range(8):  # запускаю цикл для создания 8 строк
            for col in range(5):  # запускаю вложенный цикл для создания 5 столбцов
                index = row * 5 + col  # вычисляю индекс ячейки: номер строки умножаю на 5 и прибавляю номер столбца
                cell_frame = tk.Frame(self.cells_frame, bg="white", width=30, height=30)  # создаю фрейм для каждой ячейки размером 30x30 пикселей
                cell_frame.grid(row=row, column=col, padx=3, pady=3)  # размещаю фрейм в сетке с отступами 3 пикселя
                cell_frame.grid_propagate(False)  # запрещаю фрейму изменять размер в зависимости от содержимого
                
                # Create cell indicator (square)
                cell = tk.Canvas(cell_frame, width=25, height=25, bg="gray", bd=0, highlightthickness=0)  # создаю холст для рисования ячейки размером 25x25, серый фон, без рамки
                cell.pack(padx=1, pady=1)  # размещаю холст с отступами 1 пиксель
                
                # Add cell number
                cell.create_text(12, 12, text=str(index+1), fill="white", font=("Arial", 8))  # рисую текст с номером ячейки в центре холста (координаты 12,12), белый цвет, шрифт Arial размер 8
                
                self.cell_indicators.append(cell)  # добавляю созданную ячейку в список индикаторов
        
        # Status display on right side
        status_frame = tk.Frame(self.right_frame, bg="white")  # создаю фрейм для отображения статуса в правой части с белым фоном
        status_frame.pack(fill="x", pady=5)  # размещаю фрейм статуса, растягивая по горизонтали с вертикальным отступом 5 пикселей
        
        status_label = tk.Label(status_frame, text="Статус:",  # создаю метку с текстом "Статус:"
                               bg="white", font=("Arial", 9, "bold"))  # устанавливаю белый фон и жирный шрифт Arial размер 9
        status_label.pack(anchor="w")  # размещаю метку с выравниванием по левому краю (west)
        
        self.status_msg = tk.Label(status_frame, text="Ожидание подключения к БД...",  # создаю метку для сообщения о статусе
                               fg="gray", bg="white", font=("Arial", 8))  # устанавливаю серый цвет текста, белый фон, шрифт Arial размер 8
        self.status_msg.pack(anchor="w", padx=5)  # размещаю метку с выравниванием по левому краю и горизонтальным отступом 5 пикселей
        
        # Database info
        db_frame = tk.Frame(self.right_frame, bg="white")  # создаю фрейм для информации о базе данных с белым фоном
        db_frame.pack(fill="x", pady=5)  # размещаю фрейм БД, растягивая по горизонтали с вертикальным отступом 5 пикселей
        
        db_title = tk.Label(db_frame, text="База данных:",  # создаю заголовок для секции базы данных
                          bg="white", font=("Arial", 9, "bold"))  # устанавливаю белый фон и жирный шрифт Arial размер 9
        db_title.pack(anchor="w")  # размещаю заголовок с выравниванием по левому краю
        
        self.db_label = tk.Label(db_frame, text="не подключена",  # создаю метку для отображения статуса подключения БД
                              fg="gray", bg="white", font=("Arial", 8, "italic"))  # устанавливаю серый цвет, белый фон, курсивный шрифт Arial размер 8
        self.db_label.pack(anchor="w", padx=5)  # размещаю метку с выравниванием по левому краю и горизонтальным отступом 5 пикселей
        
        # Table info
        table_frame = tk.Frame(self.right_frame, bg="white")  # создаю фрейм для информации о таблице с белым фоном
        table_frame.pack(fill="x", pady=5)  # размещаю фрейм таблицы, растягивая по горизонтали с вертикальным отступом 5 пикселей
        
        table_title = tk.Label(table_frame, text="Таблица:",  # создаю заголовок для секции таблицы
                             bg="white", font=("Arial", 9, "bold"))  # устанавливаю белый фон и жирный шрифт Arial размер 9
        table_title.pack(anchor="w")  # размещаю заголовок с выравниванием по левому краю
        
        self.table_label = tk.Label(table_frame, text="не выбрана",  # создаю метку для отображения выбранной таблицы
                                 fg="gray", bg="white", font=("Arial", 8, "italic"))  # устанавливаю серый цвет, белый фон, курсивный шрифт Arial размер 8
        self.table_label.pack(anchor="w", padx=5)  # размещаю метку с выравниванием по левому краю и горизонтальным отступом 5 пикселей
        
        # Connection indicator (replace canvas with text label)
        indicator_frame = tk.Frame(self.right_frame, bg="white")  # создаю фрейм для индикатора соединения с белым фоном
        indicator_frame.pack(fill="x", pady=5)  # размещаю фрейм индикатора, растягивая по горизонтали с вертикальным отступом 5 пикселей
        
        indicator_title = tk.Label(indicator_frame, text="Статус соединения:",  # создаю заголовок для секции статуса соединения
                                 bg="white", font=("Arial", 9, "bold"))  # устанавливаю белый фон и жирный шрифт Arial размер 9
        indicator_title.pack(anchor="w")  # размещаю заголовок с выравниванием по левому краю
        
        # Replace canvas indicator with text label
        self.conn_status = tk.Label(indicator_frame, text="Не подключено",  # создаю метку для отображения статуса соединения
                                  fg="red", bg="white", font=("Arial", 8))  # устанавливаю красный цвет текста, белый фон, шрифт Arial размер 8
        self.conn_status.pack(anchor="w", padx=5, pady=2)  # размещаю метку с выравниванием по левому краю и отступами 5 и 2 пикселя
        
        # Button frame
        self.button_frame = tk.Frame(self.right_frame, bg="white")  # создаю фрейм для кнопок с белым фоном
        self.button_frame.pack(fill="x", pady=5)  # размещаю фрейм кнопок, растягивая по горизонтали с вертикальным отступом 5 пикселей
