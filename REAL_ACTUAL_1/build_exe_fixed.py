...
# -*- coding: utf-8 -*-
"""
ИСПРАВЛЕННАЯ сборка exe с правильными путями для Windows
"""
...
def build_main_app_exe():
    ...
    # Создаем список всех файлов .db
    db_files = glob.glob("*.db")
    db_data_args = []
    for db_file in db_files:
        db_data_args.extend(['--add-data', f'{db_file};.'])
    
    # Команда для PyInstaller с правильными путями
    cmd = [
        'pyinstaller',
        '--onefile',
        '--windowed',
        ...
        '--workpath=build',
        '--specpath=build',
        # Добавляем все модули как дополнительные файлы (правильный синтаксис для Windows)
        '--add-data', 'module1.py;.',
        '--add-data', 'module2.py;.',
        '--add-data', 'module3.py;.',
        '--add-data', 'module4.py;.',
        ...
        '--add-data', 'module8.py;.',
        '--add-data', 'module9.py;.',
        # Добавляем каждую БД отдельно
    ] + db_data_args + ['MonOilStudy_portable.py']
...
def test_exe_files():
    """Тестирование созданных exe файлов"""
    print("Проверяем созданные exe файлы...")
    ...
    for exe_file in exe_files:
        ...
        if os.path.exists(exe_path):
            size = os.path.getsize(exe_path) // (1024 * 1024)  # размер в МБ
            print(f"✅ {exe_file} - {size} МБ")
        else:
...
def main():
    ...
    print("=== ИСПРАВЛЕННАЯ СБОРКА EXE ВЕРСИИ ===\n")
    ...
    if main_success and db_success:
        ...
        if test_exe_files():
            ...
            print("\n=== EXE СБОРКА УСПЕШНО ЗАВЕРШЕНА! ===")
            print(f"\n✅ EXE версия готова: {dist_dir}")
            print("\n📦 Содержимое:")
            print("- MonOilStudy.exe (основная программа с 9 модулями)")
            ...
            print("✅ Все 9 модулей встроены в exe файл!")
            
            return True
        else:
            print("❌ Exe файлы созданы с ошибками")
            return False
    else:
