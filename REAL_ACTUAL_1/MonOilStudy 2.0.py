...
class PipelineMonitoringApp:
    def __init__(self, root):
        ...
        # Добавляем ползунки прокрутки для главного окна
        ...
        self.data = {
            ...
            "Скорость откачки (литры в минуту)": (1000, 500, 1500),
            "Угол наклона трубопровода (градусы)": (1, -5, 5), 
            "Показания вибраций трубопровода (мм/с)": (2, 0, 10),
        ...
        # Экспериментальные функции с прокруткой
        ...
        self.experimental_functions_window.geometry("600x600")
        self.create_scrollable_frame(self.experimental_functions_window)
    ...
    def create_gui(self):
        # Индикаторы
        self.indicator_frame = ttk.Frame(self.scrollable_frame, borderwidth=1, relief="solid")
        self.indicator_frame.grid(row=0, column=0, padx=20, pady=20, sticky="nw")

        self.indicators = []
        for i in range(4):
            line_frame = ttk.Frame(self.indicator_frame)
            line_frame.pack(side="left", padx=5)
            line_indicators = [tk.Canvas(line_frame, width=5, height=20, bg="black") for _ in range(10)]
            for cell in line_indicators: cell.pack(side="top", pady=1)
            self.indicators.append(line_indicators)

        # Основная панель
        self.main_frame = ttk.Frame(self.scrollable_frame, borderwidth=1, relief="solid")
        ...
        for i, (label_text, data) in enumerate(self.data.items()):
            if label_text == "Нефтепродукт": continue
            value, min_value, max_value = data if isinstance(data, tuple) else data[:3]
            self.create_scale(label_text, i, value, min_value, max_value)

        # Текстовое поле с прокруткой
        self.text_frame = ttk.Frame(self.scrollable_frame, borderwidth=1, relief="solid")
        self.text_frame.grid(row=0, column=2, padx=20, pady=20, sticky="nw")
        self.text_data = tk.Text(self.text_frame, wrap="none", width=100, height=20)
        yscroll = ttk.Scrollbar(self.text_frame, orient="vertical", command=self.text_data.yview)
        xscroll = ttk.Scrollbar(self.text_frame, orient="horizontal", command=self.text_data.xview)
        self.text_data.configure(yscrollcommand=yscroll.set, xscrollcommand=xscroll.set)
        self.text_data.pack(side="left", fill="both", expand=True)
        yscroll.pack(side="right", fill="y")
        xscroll.pack(side="bottom", fill="x")

        # Отчеты с прокруткой
        self.reports_frame = ttk.Frame(self.scrollable_frame, borderwidth=1, relief="solid")
        ...
        yscroll_report = ttk.Scrollbar(self.reports_frame, orient="vertical", command=self.reports_listbox.yview)
        xscroll_report = ttk.Scrollbar(self.reports_frame, orient="horizontal", command=self.reports_listbox.xview)
        self.reports_listbox.configure(yscrollcommand=yscroll_report.set, xscrollcommand=xscroll_report.set)
        self.reports_listbox.pack(side="left", fill="both", expand=True)
        yscroll_report.pack(side="right", fill="y")
        xscroll_report.pack(side="bottom", fill="x")
        self.reports_listbox.bind("<Double-Button-1>", self.show_report_window)

        self.update_reports_list()

        self.control_frame = ttk.Frame(self.scrollable_frame, width=300, height=200, borderwidth=1, relief="solid")
        self.control_frame.grid(row=1, column=1, padx=20, pady=0, sticky="nw")

        self.create_control_panel()

        # Блок 5: изменен размер в 2 раза
        self.weather_frame = ttk.Frame(self.scrollable_frame, width=1600, height=200, borderwidth=1, relief="solid")
        ...
        # Блок 2: Статус БД
        self.status_frame = ttk.Frame(self.scrollable_frame, width=600, height=200, borderwidth=1, relief="solid")
        ...
        self.backup_frame = ttk.Frame(self.scrollable_frame, width=300, height=200, borderwidth=1, relief="solid")
        ...
        self.backup_selector_frame = ttk.Frame(self.scrollable_frame, width=300, height=200, borderwidth=1, relief="solid")
